...
const SupplierProducts = () => {
  ...
  // --- FIX SCROLL LOGIC ---
  // Plus simple, plus performant et sans bug de saut
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 pb-32 text-slate-200 font-sans selection:bg-purple-500/30">
      
      {/* ========================================
          HEADER STICKY "GLASS" - SANS SAUT DE PAGE
          ======================================== */}
      <header 
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-start">
               <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h1 className={`font-bold text-slate-100 transition-all leading-tight ${isScrolled ? 'text-lg' : 'text-2xl'}`}>
                      Mes Produits
                    </h1>
               ...
               {/* Bouton visible sur mobile uniquement ici */}
               <button 
                  onClick={() => setIsAddModalOpen(true)}
                  className="md:hidden bg-purple-600 hover:bg-purple-500 text-white p-2 rounded-lg"
                >
                  <Plus size={20} />
               </button>
            </div>
            ...
            <div className={`flex flex-col md:flex-row gap-3 w-full md:w-auto transition-all ${isScrolled ? 'md:opacity-100' : 'md:mt-0'}`}>
              ...
              <div className="flex gap-2">
                <select 
                  className="bg-slate-900/50 border border-slate-800 text-slate-300 text-sm rounded-xl px-3 py-2.5 outline-none focus:border-purple-500/50 cursor-pointer"
                  value={statusFilter}
      ...
      <div className="px-4 max-w-7xl mx-auto mt-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Total Produits', val: stats.total, icon: Package, color: 'text-slate-200', grad: 'from-slate-800 to-slate-900' },
            ...
            { label: 'En Rupture', val: stats.outOfStock, icon: AlertTriangle, color: 'text-red-400', grad: 'from-red-900/20 to-red-900/5' },
          ].map((stat, idx) => (
            <div key={idx} className={`p-4 rounded-2xl border border-slate-800 bg-gradient-to-br ${stat.grad} flex flex-col justify-between`}>
              <div className={`mb-2 p-2 w-fit rounded-lg bg-slate-950/30 ${stat.color}`}>
                <stat.icon size={18} />
              </div>
              <div>
          ...
          {filteredProducts.length === 0 ? (
            ...
            >
              ...
              <p className="text-slate-500 max-w-md mx-auto mb-8">
                {searchTerm ? `Aucun résultat pour "${searchTerm}"` : "Commencez par ajouter votre premier produit au catalogue."}
              </p>
              {!searchTerm && (
                <button onClick={() => setIsAddModalOpen(true)} className="px-6 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-xl font-bold transition-all shadow-lg shadow-purple-900/20">
                  Ajouter un produit
                </button>
              )}
            </motion.div>
                  ...
                  >
                    ...
                    <div className="relative aspect-[4/3] overflow-hidden bg-slate-950">
                      <img 
                        src={product.image} 
                        alt={product.name}
                        className={`w-full h-full object-cover transition-transform duration-700 ${!product.inStock ? 'grayscale' : 'group-hover:scale-110'}`}
                      />
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-slate-950/50 text-slate-500 text-xs uppercase tracking-wider font-semibold">
                  <tr>
                    ...
                    <th className="p-4 hidden sm:table-cell">Statut</th>
                    <th className="p-4 text-right">Votre Gain</th>
                    <th className="p-4 text-right hidden sm:table-cell">Prix Public</th>
                  ...
                  {filteredProducts.map(product => {
                     ...
                     return (
                      <tr key={product.id} className="hover:bg-slate-800/30 transition-colors group">
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            <img src={product.image} alt="" className="w-12 h-12 rounded-lg object-cover bg-slate-800" />
                            <div>
