...
export const CartProvider = ({ children }) => {
  ...
  const addToCart = (product) => {
    ...
    setCartItems(prevItems => {
      ...
      // ✅ CORRECTION IMPORTANTE: On utilise supplierPrice (nom cohérent avec Firestore)
      return [...prevItems, { 
        ...product, 
        quantity: 1, 
        cartId: Date.now(),
        supplierId: product.supplierId || null,
        supplierPrice: product.supplierPrice || 0, // ✅ CORRIGÉ: supplierPrice au lieu de buyingPrice
        price: product.price // Prix de vente public (incluant la marge)
      }];
    });
  ...
  const resolveConflictNewCart = () => {
    if (vendorConflict.pendingProduct) {
      setCartItems([{ 
        ...vendorConflict.pendingProduct, 
        quantity: 1, 
        cartId: Date.now(),
        supplierPrice: vendorConflict.pendingProduct.supplierPrice || 0 // ✅ CORRIGÉ
      }]);
      removePromo();
